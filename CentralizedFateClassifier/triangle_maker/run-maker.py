...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hitopvtw",["help", "indir=", "dotriangles", "outdir=", "dopng", "dov5", "notriangleoverwrite", "dov45"])
    ...
    outfilename_cat = join(outdir, "concat_" + os.path.basename(indir) + ".csv")
    outfilename_triangle = join(outdir, "triangles_" + os.path.basename(indir) + ".csv")

    outfile_cat = open(outfilename_cat, 'w')
    outfile_triangle = open(outfilename_triangle, 'w')


    outfile_cat.write("original node, g.m, g.h, g.w, first descendant id, a.m, a.h, a.w, second descendant id, b.m, b.h, b.w, P_subfunctionlization, P_conservation, P_newfunctionlizatoin, P_pseudogenization, P_specialization, P_dblneo, orig_file\n")
    outfile_triangle.write("subfunc, neofunc, cons, pseudo, spec, dblneo, mg, hg, wg, ma, ha, wa, mb, hb, wb, fitness_g, fitness_a, fitness_b\n")

    csvs_dir = indir
    if dov5:
        print("Recalculating probabilities from " + indir + " to " + outdir + " (v5)")
        recalc_csv(indir, outdir, "5")
        csvs_dir = outdir

    if dov45:
        print("Recalculating probabilities from " + indir + " to " + outdir + " (v45)")
        recalc_csv(indir, outdir, "45")
        csvs_dir = outdir
        ...
        if "dups_fates_probablities.csv" in f:
            ...
            for (r, row) in enumerate(rows):
                if len(row) == 0:
                    ...
                    outfile_triangle.write(cut(row['P_specialization']) + "," + cut(dblneo_str) + ",")
