...
#!/usr/bin/python
# python triangle_maker_multiproc.py --mg 0.34 --hg 0.78 --wg 0.03 --ma 0.26 --ha 1 --wa 0.025 --mb 0.4 --hb 1 --wb 0.025 --pneo 0 --psubfunc 0 --pcons 0 --pspec 1 --ppseudo 0 --iga 1 --iag 1 --igb 1 --ibg 1 --igaplusb 1 --iaplusbg 1 --pa 1 --pb 1
...
def generatePdfForRow(params):
    ...
    if overwrite or not os.path.isfile(outfilename):
        ...
        cmd = f"python triangle_maker_multiproc.py --mg {row['mg']} --hg {row['hg']} --wg {row['wg']} --ma {row['ma']} --ha {row['ha']} --wa {row['wa']} --mb {row['mb']} --hb {row['hb']} --wb {row['wb']}"
        cmd += f" --pneo {row['neofunc']} --psubfunc {row['subfunc']} --pcons {row['cons']} --pspec {row['spec']} --ppseudo {row['pseudo']}"

        #cmd += f"--pconsloss {row['cons_loss']} --pneoloss {row['neo_loss']}"
        #cmd += f" --iga {row['ig_a']} --iag {row['ia_g']} --igb {row['ig_b']} --ibg {row['ib_g']} --igaplusb {row['ig_a_plus_b']} --iaplusbg {row['ia_plus_b_g']} --pa {row['pa']}"
        #cmd += --pb {row['pb']} 
        cmd += f" --id {r+2} "
...
def parse_csv(filename):
    ...
    for (r, row) in enumerate(rows):
        if len(row) != 0:
            allinputs.append( [row, r, outdir, overwrite] )
        
    with Pool(10) as p:
        p.map(generatePdfForRow, allinputs)
...
def main(argv):
    ...
    base_template_filename = "tikztriangle_template.tex"
    ...
    try:
        opts, args = getopt.getopt(argv,"hs",["help", "startfile", "mg=", "hg=", "wg=", "ma=", "ha=", "wa=", "mb=", "hb=", "wb=", "pneo=", "psubfunc=", "pcons=", "pspec=", "ppseudo=", "gcolor=", "acolor=", "bcolor=", "batch=", "outfile=", "outdir=", "id=", "nooverwrite"])
    ...
    for opt, arg in opts:
        if opt in ['-h', '--help']:
            ...
            print ('triangle_maker_multiproc.py --mg value1 --hg value2 --wg value3 --ma value4 --ha value5 --wa value6 --mb value7 --hb value8 --wb value9 --pneo pro1 --psubfunc pro2 --pcons pro3 --pspec pro4 --ppseudo pro5 --gcolor color1 --acolor color2 --bcolor color3')
            print('To generate triangle files in batch from a csv:')
            print('triangle_maker_multiproc.py --batch=csvfile --outdir=outdir')
            sys.exit()
        elif opt in ("--batch"):
            ...
            ppseudo = float(arg)
        elif opt in ["--pconsloss"]:
            pconsloss = float(arg)
        elif opt in ["--pneoloss"]:
            pneoloss = float(arg)
        elif opt in ["--iga"]:
            iga = float(arg)
        elif opt in ["--iag"]:
            ...
            ibg = float(arg)
        elif opt in ["--igaplusb"]:
            igaplusb = float(arg)
        elif opt in ["--iaplusbg"]:
            iaplusbg = float(arg)
        elif opt in ["--pa"]:
            pa = float(arg)
        elif opt in ["--pb"]:
    ...
    template_filename = base_template_filename
    template_filename_out = "tikz_test_out.tex"
    prevcwd = os.getcwd()
    if global_id != 0:
       template_filename = str(global_id) + ".tex"
       template_filename_out = str(global_id) + "_out.tex"
       os.makedirs("work", exist_ok=True)
       shutil.copyfile(base_template_filename, "work/" + template_filename)
       os.chdir("work")
    ...
    #filedata = filedata.replace('<!pconsloss>', str(pconsloss))
    #filedata = filedata.replace('<!pneoloss>', str(pneoloss))
    
    #filedata = filedata.replace('<!iga>', str(iga))
    #filedata = filedata.replace('<!iag>', str(iag))
    #filedata = filedata.replace('<!igb>', str(igb))
    #filedata = filedata.replace('<!ibg>', str(ibg))
    #filedata = filedata.replace('<!igaplusb>', str(igaplusb))
    #filedata = filedata.replace('<!iaplusbg>', str(iaplusbg))
    #filedata = filedata.replace('<!pa>', str(pa))
    #filedata = filedata.replace('<!pb>', str(pb))
    ...
    if outputfilename != '':
        shutil.copyfile(prefix + pdffilename, outputfilename) 
    else:
