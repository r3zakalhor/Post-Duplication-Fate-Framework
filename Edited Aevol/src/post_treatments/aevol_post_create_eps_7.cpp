...
void draw_genetic_unit_with_CDS(Individual_7* indiv, char* directoryName);
...
// the others are gray.
void draw_genetic_unit_with_mRNAs(Individual_7* indiv, char* directoryName);
...
//     }
  ...
  exp_manager->exp_m_7_->evaluate(indiv_7,indiv->w_max(),1);
...
//       else cout << "This path is a file." << endl;
  ...
  fflush(stdout);
  draw_phenotype(indiv_7, exp_manager->exp_m_7_->target, directory_name);
  printf("OK\n");
#endif

  printf("Creating the EPS file with the CDS of the chosen individual... ");
  fflush(stdout);
  draw_genetic_unit_with_CDS(indiv_7, directory_name);
  printf("OK\n");

  printf("Creating the EPS file with the mRNAs of the chosen individual... ");
  fflush(stdout);
  draw_genetic_unit_with_mRNAs(indiv_7, directory_name);
  printf("OK\n");
...
void draw_phenotype(Individual_7* indiv, AbstractFuzzy_7* target,
  ...
  if (indiv->exp_m_->exp_s()->get_fuzzy_flavor() == 1)
    for (const auto& p: ((Vector_Fuzzy*)indiv->phenotype)->points())
      fprintf(drawingfile, "%lf %lf lineto\n", margin + scale * p.x, margin + scale * p.y);
  else {
    for (int i=0; i < ((Discrete_Double_Fuzzy*)indiv->phenotype)->length(); i++) {

      float xi =  (i / (float)
                      ((Discrete_Double_Fuzzy*) indiv->phenotype)->length());
      fprintf(drawingfile, "%lf %lf lineto\n", margin +
                                               scale * xi, margin + scale *
                                                                    ((Discrete_Double_Fuzzy*) indiv->phenotype)->points()[i]);
    }
  }
  fprintf( drawingfile, "stroke\n" );
  ...
  if (indiv->exp_m_->exp_s()->get_fuzzy_flavor() == 1)
    for (const auto& p: ((Vector_Fuzzy*)target)->points())
      fprintf(drawingfile, "%lf %lf lineto\n", margin + scale * p.x, margin + scale * p.y);
  else {
    for (int i=0; i < ((Discrete_Double_Fuzzy*)target)->length(); i++) {
